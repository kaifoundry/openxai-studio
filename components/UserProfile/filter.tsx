...
export const CategoryTag: React.FC<{
  ...
  return (
    <motion.div
      ...
      className={`flex cursor-pointer items-center gap-4 rounded-xl px-4 py-3 shadow-sm 
        md:gap-2 md:px-6 xl:gap-2 xl:py-2 2xl:gap-2 2xl:py-2 3xl:gap-2 3xl:py-2
        ${
          isSelected
    ...
    >
      ...
      <div className="font-[500] text-[#9B9DA0] md:text-[10px] xl:text-[14px] 2xl:text-[13px] 3xl:text-[13px]">
        {getItemKey(tag)}
      </div>
    </motion.div>
  );
...
export const ChainIcon: React.FC<{
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className={`flex cursor-pointer items-center justify-between rounded-lg border border-[#EBF2FF] bg-[#F6F9FF] 
        md:px-3 md:py-1 xl:px-3 xl:py-1 2xl:px-3 2xl:py-1 3xl:px-4 3xl:py-3
...
const Filter: React.FC = () => {
  ...
  return (
    <div className="transition duration-700 delay-300">
      ...
      <div className="mt-10 flex flex-col gap-6 md:mt-0">
        ...
        >
          {ChainData.map((chain, index) => (
            <ChainIcon key={chain.name} chain={chain} index={index} />
          ))}
        </motion.div>
      </div>
      <hr className="w-[95%] bg-[#A3A3A3] my-3"/>
      <div className="mt-10 flex flex-col gap-6 md:mt-0">
      <motion.h2
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
      ...
      >
        {tagsData.slice(0, visibleCount).map((tag, index) => {
          ...
          return <CategoryTag key={tag.label} tag={tag} index={delayIndex} />;
        })}
